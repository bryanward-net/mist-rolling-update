...
from mist import Mist, MistHelpers, MistWebsocket
import config
...
parser = argparse.ArgumentParser(description="Upgrade FW of Mist APs, one at a time")
parser.add_argument("--token", help="Mist API Token.  Can also use env var MIST_TOKEN")
parser.add_argument("--orgid", required=False, help="Organization ID to run against.  Can also use env var MIST_ORGID")
parsergroup_site = parser.add_mutually_exclusive_group(required=True)
parsergroup_site.add_argument("--site", required=False, help="Site name to upgrade")
parsergroup_site.add_argument("--siteid", required=False, help="Site ID to upgrade")
#parser.add_argument("--version", required=False, help="Version to upgrade to.  Otherwise, use values defined in config.py")
#parser.add_argument("--connected", default=False, help="Only upgrade connected APs, disconnected APs upgrade when they connect", action="store_true")
#parser.add_argument("--skip", help="AP device id to skip")
#parser.add_argument("--numretries", default=5, help="Number of times to reattempt an operation.  Default 5.")
#parser.add_argument("--waittime", default=5, help="Time in seconds to wait before retrying a failed operation.  Default 5.")
#parser.add_argument("--sleeptime", default=120, help="Time in seconds to wait before refreshing the status of an AP.  Default 120.")
parser.add_argument('--delay', required=False, help='Additional time to wait between devices', default=0, dest='delay')
parser.add_argument('--debug', help='Debug Logging Enabled', default=False, action='store_true', dest='debug')
...
def rollingupgrade(ev):
        ...
        for device in devices:
            ...
            if device['version'] != config.versions[device['model']]['version']:
                ...
                status = h.update_fw(SITEID, device, config.versions[device['model']]['version'])
                        ...
                        if msg['event'] == "data" and "data" in msg:
                            ...
                            if "mac" in data and device['mac'] == data['mac']:
                                ...
                                if "status" in data and data['status'] == "upgrading" and "upgrading" in data and data['upgrading'] == True and "progress" in data:
                                    logging.info("Update of device {0} [{1}] is in progress: {2}%".format(device['name'], device['mac'], data["progress"]))
                                else:
                                    #Poll device status
                                ...
                                if device['version'] == config.versions[device['model']]['version']:
                                    ...
                                    logging.info("Update of device {0} [{1}] Complete!  Now at version {2}".format(device['name'], device['mac'], device['version']))
                                    working=False
                ...
                logging.info("Firmware on device {0} [{1}] is already at version {2}".format(device['name'], device['mac'], device['version']))
